...
class hydration:
    ...
    def __init__(self, workspace={}, inputs={}, constants={}):

        if workspace and isinstance(workspace, dnpdata_collection):
            return self.odnp_dnplab(workspace)
        elif inputs:
            return self.odnp_dicts(inputs, constants)
        else:
            raise ValueError(
                "give either a dnpdata_collection OR inputs dictionary and optional constants dictionary"
            )

    def calculate_smax(self, spin_C=False):
    ...
    def calculate_ksigma_array(self, powers=False, ksigma_smax=95.4, p_12=False):
    ...
    def calculate_ksigma(self, ksigma_sp=False, powers=False, smax=1):
        ...
        ksigma_fit = self.calculate_ksigma_array(powers, ksigma_smax, p_12)

        ksigma = ksigma_smax / smax

        return ksigma, ksigma_stdd, ksigma_fit

    def calculate_xi(self, tcorr=54, omega_e=0.0614, omega_H=9.3231e-05):
    ...
    def calculate_tcorr(self, coupling_factor=0.27, omega_e=0.0614, omega_H=9.3231e-05):
        ...
        result = optimize.root_scalar(
            lambda tcorr: self.calculate_xi(tcorr, omega_e=omega_e, omega_H=omega_H)
            - coupling_factor,
            method="brentq",
            bracket=[1, 1e5],
        )
    ...
    def _residual_Ep(
        ...
        return E_array - self.calculate_uncorrected_Ep(
            uncorrected_xi=x[0],
            p_12_unc=x[1],
            E_powers=E_powers,
            T10=T10,
    ...
    def odnp_dicts(self, inputs={}, constants={}):
        ...
        if inputs["smax_model"] == "tethered":
            ...
            s_max = self.calculate_smax(inputs["spin_C"])  # from:
        ...
        ksigma, ksigma_stdd, ksigma_fit = self.calculate_ksigma(
        ...
        tcorr = self.calculate_tcorr(coupling_factor, omega_e, omega_H)
        ...
        xi_unc, p_12_unc = self.calculate_uncorrected_xi(
            inputs["E_array"],
            inputs["E_powers"],
            inputs["T10"],
            inputs["T100"],
        ...
        uncorrected_Ep = self.calculate_uncorrected_Ep(
            xi_unc,
            p_12_unc,
            inputs["E_powers"],
            inputs["T10"],
    ...
    def odnp_dnplab(self, workspace):
        ...
        if "hydration_inputs" in workspace.keys():
            ...
            results = self.odnp_dicts(odnp_inputs, odnp_constants)
